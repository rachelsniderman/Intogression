...
vcf <- fread("zcat ~/fish/variants/all.filtered.vcf.gz| grep -v '^#'", header=FALSE, showProgress=TRUE)
...
write.table(vcf, "~/fish/variants/all.chrom.vcf", col.names=FALSE, row.names=FALSE, quote=FALSE)
